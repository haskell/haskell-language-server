...
-- 'VirtualFileTree'. The continuation receives the per-test cache directory.
...
-- via 'argsGetHieDbLoc' and 'getCacheDirs'.
...
withTemporaryDataAndCacheDirectory act = do
    ...
    let runTestInDir action = case cleanupTempDir of
            Just val | val /= "0" -> do
                (tempDir, cacheDir, _) <- setupTemporaryTestDirectories testRoot
                a <- action tempDir cacheDir
                logWith helperRecorder Debug LogNoCleanup
                pure a

            _ -> do
                (tempDir, cacheDir, cleanup) <- setupTemporaryTestDirectories testRoot
                a <- action tempDir cacheDir `finally`  cleanup
                logWith helperRecorder Debug LogCleanup
                pure a
    runTestInDir $ \tmpDir' cacheDir -> do
    ...
    where
        ...
        setupTemporaryTestDirectories testRoot = do
            ...
            (tempCacheDir, cleanup2) <- newTempDirWithin testRoot
            pure (tempTestCaseDir, tempCacheDir, cleanup1 >> cleanup2)
