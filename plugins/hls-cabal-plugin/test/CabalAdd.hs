...
cabalAddModuleTests =
    testGroup
        "Add Module"
        [ runHaskellTestCaseSession "Code Actions - Can add module to benchmark" ("cabal-add-module" </> "library") $ do
        ...
        , runHaskellTestCaseSession "Code Actions - Can add module to executable" ("cabal-add-module" </> "library") $ do
            let compName = CExeName "test"
            pd <- generateAddDependencyTestSession "test.cabal" "Main.hs" compName
            checkModuleAddedTo pd "Main" $ compName
        , runHaskellTestCaseSession "Code Actions - Can add module to test-suite" ("cabal-add-module" </> "library") $ do
            let compName = CTestName "test2"
            pd <- generateAddDependencyTestSession "test.cabal" "Main.hs" compName
            checkModuleAddedTo pd "Main" $ compName
        , runHaskellTestCaseSession "Code Actions - Can add module to library" ("cabal-add-module" </> "library") $ do
            let compName = CLibName $ LSubLibName "test3"
            pd <- generateAddDependencyTestSession "test.cabal" "Main.hs" compName
            checkModuleAddedTo pd "Main" $ compName
        , runHaskellTestCaseSession "Code Actions - Can add module to main library" ("cabal-add-module" </> "library") $ do
