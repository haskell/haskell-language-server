...
haskellInteractionDescriptor recorder plId =
  (defaultPluginDescriptor plId "Provides the cabal-add code action in haskell files")
    { pluginHandlers =
        mconcat
          [ mkPluginHandler LSP.SMethod_TextDocumentCodeAction cabalAddPackageCodeAction
          , mkPluginHandler LSP.SMethod_TextDocumentCodeAction $ cabalAddModuleCodeAction recorder
          ]
    , pluginCommands =
        [ PluginCommand CabalAdd.cabalAddPackageCommandId "add a dependency to a cabal file" (CabalAdd.addPackageCommand cabalAddRecorder)
...
cabalAddPackageCodeAction :: PluginMethodHandler IdeState 'LSP.Method_TextDocumentCodeAction
cabalAddPackageCodeAction state plId (CodeActionParams _ _ (TextDocumentIdentifier uri) _ CodeActionContext{_diagnostics = diags}) = do
