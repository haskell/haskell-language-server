...
#if APPLY_REFACT
  | LogGeneratedIdeas NormalizedUri [[Refact.Refactoring Refact.SrcSpan]]
#endif
...
applyHint recorder ide nuri mhint verTxtDocId =
  runExceptT $ do
    ...
    case mbOldContent of
      Nothing -> throwError (PluginInternalError $ "No contents for the respected Uri: " <> getUri (fromNormalizedUri nuri))
      Just oldContent -> do
        res <-
            liftIO $ withSystemTempFile "hls-hlint" $ \temp h -> do
                hClose h
