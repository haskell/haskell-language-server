...
import Development.IDE (Recorder, WithPriority, getFileContents, usePropertyAction)
import Development.IDE.Core.FileStore (getVersionedTextDoc, getVersionedTextDocForNormalizedFilePath)
...
import Development.IDE.GHC.ExactPrint qualified as E
import Development.IDE.Import.DependencyInformation (DependencyInformation (depPathIdMap), pathToIdMap)
...
data Log
  = LogExactPrint E.Log
  | LogRenameModule NormalizedFilePath
  deriving (Show)

descriptor :: Recorder (WithPriority E.Log) -> PluginId -> PluginDescriptor IdeState
descriptor recorder pluginId =
  mkExactprintPluginDescriptor recorder $
    (defaultPluginDescriptor pluginId "Provides renaming of Haskell identifiers")
      { pluginHandlers =
          mconcat
            ...
            , mkPluginHandler SMethod_WorkspaceWillRenameFiles $ moduleRenameProvider recorder
            ]
...
prepareRenameProvider state _pluginId (PrepareRenameParams (TextDocumentIdentifier uri) pos _progressToken) = do
  ...
  let
    renameValid = not $ null namesUnderCursor
...
renameProvider state pluginId (RenameParams _prog (TextDocumentIdentifier uri) pos newNameText) = do
  ...
  indirectOldNames <-
    concat . filter ((> 1) . length)
      <$> mapM (uncurry (getNamesAtPos state) <=< locToFilePos) directRefs
  let
    oldNames = filter matchesDirect indirectOldNames ++ directOldNames
     where
  ...
  case oldNames of
    ...
    _ -> do
      ...
      -- Perform rename
      let
        newName = mkTcOcc $ T.unpack newNameText
...
moduleRenameProvider :: Recorder (WithPriority Log) -> IdeState -> PluginId -> RenameFilesParams -> ExceptT PluginError (HandlerM Config) (WorkspaceEdit |? Null)
moduleRenameProvider recorder state plId (RenameFilesParams renames) = do
  caps <- lift pluginGetClientCapabilities
  edits <-
    forM renames $ \(FileRename oldUri newUri) -> do
      oldHaskellFilePath <- fmap toNormalizedFilePath $ uriToFilePathE $ Uri oldUri
      newHaskellFilePath <- uriToFilePathE $ Uri newUri
      knownTargets <- liftIO $ readTVarIO $ knownTargetsVar $ shakeExtras state
      verTextDocId <- runActionE "cabal-plugin.getUriContents" state $ lift $ getVersionedTextDocForNormalizedFilePath oldHaskellFilePath
      rangeToRename <-
        maybeToExceptT PluginStaleResolve $
          MaybeT $
            liftIO $
              moduleNameRange state oldHaskellFilePath
      depInfo <- runActionE "moduleRenameProvider" state $ lift $ useWithSeparateFingerprintRule_ GetModuleGraphTransDepsFingerprints GetModuleGraph oldHaskellFilePath
      fpId <-
        maybeToExceptT PluginStaleResolve $
          MaybeT $
            pure $
              HashMap.lookup oldHaskellFilePath $
                pathToIdMap $
                  depPathIdMap depInfo

      let
        edit = mkTextEditInRange (newModule) verTextDocId rangeToRename
      pure undefined
  pure undefined

guessModuleName state uri = do
  nfp <- getNormalizedFilePathE uri
  fp <- uriToFilePathE uri

  contents <- liftIO $ runAction "ModuleName.getFileContents" state $ getFileContents nfp
  let
    emptyModule = maybe True (T.null . T.strip . Rope.toText) contents

  correctNames <- mapExceptT liftIO $ pathModuleNames recorder state nfp fp
  logWith recorder Debug (CorrectNames correctNames)
  let
    bestName = minimumBy (comparing T.length) <$> NE.nonEmpty correctNames
  ...
  case (bestName, statedNameMaybe) of
    ...
    (Just bestName, Nothing)
      | emptyModule ->
          let
            code = "module " <> bestName <> " where\n"
          in
            pure [Replace uri (Range (Position 0 0) (Position 0 0)) code code]
...
failWhenImportOrExport state nfp refLocs names = do
  ...
  let
    hsMod = unLoc $ pm_parsed_source pm
  case (unLoc <$> hsmodName hsMod, hsmodExports hsMod) of
    ...
    (_, Just (L _ exports))
      | any ((`HS.member` refLocs) . unsafeSrcSpanToLoc . getLoc) exports ->
...
replaceRefs ::
  ...
  ParsedSource ->
  ParsedSource
replaceRefs newName refs =
  everywhere $
    -- there has to be a better way...
    mkT (replaceLoc @AnnListItem)
      `extT`
...
moduleNameRange :: IdeState -> NormalizedFilePath -> IO (Maybe Range)
moduleNameRange state nfp = runMaybeT $ do
  ...
  L (locA -> (RealSrcSpan l _)) _ <- MaybeT . pure . hsmodName . unLoc $ pm_parsed_source pm
...
removeGenerated HAR{..} =
 ...
 where
  ...
  goAst (Node (SourcedNodeInfo sniMap) sp children) =
    let
      sourceOnlyNodeInfos = SourcedNodeInfo $ M.delete GeneratedInfo sniMap
    in
      Node sourceOnlyNodeInfos sp $ map goAst children
