...
renameProvider state pluginId (RenameParams (TextDocumentIdentifier uri) pos _prog newNameText) =
    pluginResponse $ do
        ...
        let oldNames = (filter matchesDirect indirectOldNames) ++ directOldNames
            -- We need  to filter out names which do not have the same occname
            -- as the original names because GHC 9.4+ puts implicit bindings
            -- like dictionary bindings (`$fShow`) at these source spans.
