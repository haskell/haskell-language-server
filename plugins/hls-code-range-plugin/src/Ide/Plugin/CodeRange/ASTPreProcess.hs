...
identifierForTypeSig node =
  ...
  where
    ...
    extractIdentifier node' =
      sourceNodeInfo node'
        >>= ( fmap fst
                . find (\(_, detail) -> TyDecl `Set.member` identInfo detail)
                . Map.toList
                . nodeIdentifiers
            )
...
isIdentADef outerSpan (span, detail) =
  realSrcSpanStart span >= realSrcSpanStart outerSpan
    && realSrcSpanEnd span <= realSrcSpanEnd outerSpan
    && isDef
  where
    isDef :: Bool
