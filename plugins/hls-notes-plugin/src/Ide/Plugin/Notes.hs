...
getNote nfp state (Position l c)
    | getSourceFileOrigin nfp == FromDependency = pure Nothing
    | otherwise = do
        contents <-
...
findNotesInFile file recorder
    | getSourceFileOrigin file == FromDependency = pure (Just (HM.empty, HM.empty))
    | otherwise = do
...
autocomplete state _ params = do
  ...
  case uriToNormalizedFilePath nuri of
    Just nfp | getSourceFileOrigin nfp == FromDependency -> pure $ InL []
    _ -> do
      contents <-
        liftIO $
