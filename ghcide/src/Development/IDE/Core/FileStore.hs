...
setFileModified :: Recorder (WithPriority Log)
                ...
                -> IO [Key]
                -> IO ()
setFileModified recorder vfs state saved input actionBefore = do
    restartShakeSession (shakeExtras state) vfs (fromNormalizedFilePath (inputFilePath input) ++ " (modified)") [] $ do
        keys<-actionBefore
        return (toKey GetModificationTime (SomeFileHaskellInput input):keys)
    case input of
      SomeProjectHaskellInput projectInput -> do
        ...
        let checkParents = case doCheckParents of
              ...
              _           -> False
        when checkParents $
          typecheckParents recorder state projectInput
      SomeNonProjectHaskellInput{} -> pure ()
