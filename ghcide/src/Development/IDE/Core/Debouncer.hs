...
newAsyncDebouncer = Debouncer . asyncRegisterEvent <$> newIORef Map.empty
...
-- to mask if required.
asyncRegisterEvent :: (Eq k, Hashable k) => IORef (HashMap k (Async ())) -> Seconds -> k -> IO () -> IO ()
asyncRegisterEvent d 0 k fire = do
  join $ atomicModifyIORef' d $ \m ->
...
asyncRegisterEvent d delay k fire = mask_ $ do
    a <- asyncWithUnmask $ \unmask -> unmask $ do
        sleep delay
        fire
        atomicModifyIORef_ d (Map.delete k)
    join $
        atomicModifyIORef' d $ \m ->
