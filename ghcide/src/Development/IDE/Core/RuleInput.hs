...
-- @
--
-- This makes both the rule definition and its callers use a
-- 'ProjectHaskellInput' when working with @GetParsedModule@.
-}
...
`-- 'SomeFileInput'                  ->  has a 'NormalizedFilePath'
    ...
    `-- 'SomeHaskellInput'           ->  all Haskell files
        ...
        `-- 'ProjectHaskellInput'    ->  all other Haskell files
@
...
-- Rules with 'NoInput' ruletype must be treated as global rules.
