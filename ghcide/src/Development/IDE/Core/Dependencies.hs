...
indexDependencyHieFiles recorder se hscEnv = do
    ...
    -- If the .hls directory does not exists, it may have been deleted
    -- In this case, delete the indexed source file for all
    ...
    where
        ...
        deleteMissingDependencySources =
            atomically $ writeTaskQueue (indexQueue $ hiedbWriter se) $
                \withHieDb ->
                    withHieDb $ \db ->
        ...
        indexPackageHieFiles (Package package) modules = do
            let pkgLibDir :: FilePath
                pkgLibDir = case GHC.unitLibraryDirs package of
                    []               -> ""
                    (libraryDir : _) -> unpack libraryDir
            ...
            case moduleRows of
                ...
                -- so go ahead and index all of the modules
...
calculateTransitiveDependencies unitInfoMap allDependencies newDependencies
    ...
    where
        ...
        -- Get the dependencies of the newDependencies. Then the nextNew dependencies
        -- will be the set difference of the dependencies we have so far (all dependencies),
        -- and the dependencies of the newDependencies.
