...
type FileExistsMap = STM.Map (Path Abs NormalizedFilePath) Bool
...
modifyFileExists :: IdeState -> [(Path Abs NormalizedFilePath, FileChangeType)] -> IO [Key]
...
getFileExists :: Path Abs NormalizedFilePath -> Action Bool
  ...
  let globs = watchedGlobs opts
      ...
      isWatched = if supportsWatchedFiles
        then \f -> do
            ...
            return $ isWF && fpMatches (fromAbsPath f)
...
fileExistsRulesFast :: Recorder (WithPriority Log) -> (Path Abs NormalizedFilePath -> Action Bool) -> Rules ()
...
fileExistsFast :: Path Abs NormalizedFilePath -> Action (Maybe BS.ByteString, Maybe Bool)
...
fileExistsSlow :: Path Abs NormalizedFilePath -> Action (Maybe BS.ByteString, Maybe Bool)
