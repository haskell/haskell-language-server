...
getModIface ide item = do
  let nPath = artifactFilePath <$> snd item
  case nPath of 
    (Just path) -> do 
      file <- runIdeAction "Completion" (shakeExtras ide) $ do
        fmap fst <$> useWithStaleFast GetModIface path
      pure $ fmap hirModIface file
