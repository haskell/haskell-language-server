...
runGhcideCodeAction state (CodeActionParams _ _ (TextDocumentIdentifier uri) _range CodeActionContext{_diagnostics=List diags}) codeAction = do
  ...
  caaContents <- onceIO $ runRule GetFileContents >>= \case
      Just (_, txt) -> pure txt
      _             -> pure Nothing
   ...
   <$>
     sequence
       [runReaderT codeAction caa |
          caaDiagnostic <- diags, let caa = CodeActionArgs {..}]
...
mkGhcideCAPlugin  codeAction plId=
  (defaultPluginDescriptor plId)
    {
      pluginHandlers = mkPluginHandler STextDocumentCodeAction $ \state _ params@(CodeActionParams _ _ (TextDocumentIdentifier uri) _ CodeActionContext{_diagnostics=List diags}) -> do
        results <- runGhcideCodeAction state params codeAction
        pure $ Right $ List [ mkCA title kind isPreferred diags edit |
...
instance ToTextEdit Rewrite where
  toTextEdit CodeActionArgs {..} rw =fmap (fromMaybe []) $ runMaybeT $ do
    df <- MaybeT caaDf
...
toCodeAction2  get f = ReaderT $  \caa -> get caa >>= \case
...
instance ToCodeAction r => ToCodeAction (ParsedSource -> r) where
  toCodeAction  f = ReaderT $  \caa@CodeActionArgs {caaAnnSource = x} -> x >>= \case
