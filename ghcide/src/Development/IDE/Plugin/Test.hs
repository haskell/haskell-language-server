...
testRequestHandler s (GetInterfaceFilesDir file) = liftIO $ do
    let nfp = mkAbsPath $ fromUri $ toNormalizedUri file
...
testRequestHandler s GetFilesOfInterest = do
    ...
    return $ Right $ toJSON $ map fromAbsPath $ HM.keys ff
...
parseAction :: CI String -> Path Abs NormalizedFilePath -> Action (Either Text Bool)
