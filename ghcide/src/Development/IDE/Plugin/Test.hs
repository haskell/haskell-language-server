...
testRequestHandler s (GetInterfaceFilesDir file) = liftIO $ do
    case runExcept $ classifyAsProjectHaskell file of
...
withSingleProjectFile :: SomeFileInput -> (ProjectHaskellInput -> Action (Either Text Bool)) -> Action (Either Text Bool)
withSingleProjectFile (SomeFileHaskellInput (SomeProjectHaskellInput pFile)) action = action pFile
withSingleProjectFile _ _ = pure $ Right False

withSingleHaskellFile :: SomeFileInput -> (SomeHaskellInput -> Action (Either Text Bool)) -> Action (Either Text Bool)
withSingleHaskellFile (SomeFileHaskellInput hFile) action = action hFile
withSingleHaskellFile _ _                                 = pure $ Right False

parseAction :: CI String -> SomeFileInput -> Action (Either Text Bool)
parseAction "typecheck" fp = withSingleProjectFile fp $ \pFile -> Right . isJust <$> use TypeCheck pFile
parseAction "getLocatedImports" fp = withSingleProjectFile fp $ \pFile -> Right . isJust <$> use GetLocatedImports pFile
parseAction "getmodsummary" fp = withSingleProjectFile fp $ \pFile -> Right . isJust <$> use GetModSummary pFile
parseAction "getmodsummarywithouttimestamps" fp = withSingleProjectFile fp $ \pFile -> Right . isJust <$> use GetModSummaryWithoutTimestamps pFile
parseAction "getparsedmodule" fp = withSingleProjectFile fp $ \pFile -> Right . isJust <$> use GetParsedModule pFile
parseAction "ghcsession" fp = withSingleProjectFile fp $ \pFile -> Right . isJust <$> use GhcSession pFile
parseAction "ghcsessiondeps" fp = withSingleProjectFile fp $ \pFile -> Right . isJust <$> use GhcSessionDeps pFile
parseAction "gethieast" fp = withSingleHaskellFile fp $ \hFile -> Right . isJust <$> use GetHieAst hFile
...
withProjectFile fps action
    | Just pFiles <- traverse projectFile fps = action pFiles
    | otherwise = pure $ Right $ False <$ fps
  where
...
withHaskellFile fps action
    | Just hFiles <- traverse haskellFile fps = action hFiles
    | otherwise = pure $ Right $ False <$ fps
  where
    haskellFile (SomeFileHaskellInput hFile) = Just hFile
    haskellFile _                            = Nothing

parseActions :: CI String -> [SomeFileInput] -> Action (Either Text [Bool])
parseActions "typecheck" fps = withProjectFile fps $ \hFile -> Right . fmap isJust <$> uses TypeCheck hFile
parseActions "getLocatedImports" fps = withProjectFile fps $ \hFile -> Right . fmap isJust <$> uses GetLocatedImports hFile
parseActions "getmodsummary" fps = withProjectFile fps $ \hFile -> Right . fmap isJust <$> uses GetModSummary hFile
parseActions "getmodsummarywithouttimestamps" fps = withProjectFile fps $ \hFile -> Right . fmap isJust <$> uses GetModSummaryWithoutTimestamps hFile
parseActions "getparsedmodule" fps = withProjectFile fps $ \hFile -> Right . fmap isJust <$> uses GetParsedModule hFile
parseActions "ghcsession" fps = withProjectFile fps $ \hFile -> Right . fmap isJust <$> uses GhcSession hFile
parseActions "ghcsessiondeps" fps = withProjectFile fps $ \hFile -> Right . fmap isJust <$> uses GhcSessionDeps hFile
parseActions "gethieast" fps = withHaskellFile fps $ \hFile -> Right . fmap isJust <$> uses GetHieAst hFile
